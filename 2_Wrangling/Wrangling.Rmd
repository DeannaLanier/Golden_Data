...
```{r}
MAF_part2 = MAF_part1[order(MAF_part2$id),]
```
There are no unique identifiers in either dataframe. Create unique IDs by combining ID and VISITAGE
...
skim(GRLS_Data, visit.age, calendaryear, bcs, pbcs, weight_kg)
...
#Save
```{r}
saveRDS(Diet_Data_Clean,death_data, file = "~/Dropbox (Edison_Lab@UGA)/Projects/vet/Deanna/Golden_Data/CleanData.rds")
```
